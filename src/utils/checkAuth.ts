...
import { createCookieOption, decipherToken } from "../utils";
    ...
    if (token) {
        try {
            ...
            if (user && user.status === Status.Block) {
                ...
                return res.clearCookie(Secret.NameToken, createCookieOption()).status(403).send({
            ...
            if (!user) {
                ...
                return res.clearCookie(Secret.NameToken, createCookieOption()).status(404).send({
